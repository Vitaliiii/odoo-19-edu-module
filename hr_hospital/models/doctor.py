...
class HospitalDoctor(models.Model):
    ...
    rating = fields.Float(string='Rating', default=0.0, help="Doctor's rating from 0.00 to 5.00")
    education_country_id = fields.Many2one('res.country', string='Country of Education')
    schedule_ids = fields.One2many('hr.hospital.doctor.schedule', 'doctor_id', string='Work Schedule')
    patient_ids = fields.One2many('hr.hospital.patient', 'doctor_id', string='Patients')

    _sql_constraints = [
        ('unique_license_number', 'UNIQUE(license_number)', 'The license number must be unique!'),
        ('check_rating_range', 'CHECK(rating >= 0 AND rating <= 5)', 'Rating must be between 0.00 and 5.00!'),
    ]
    ...
    def _compute_experience_years(self):
        ...
        for rec in self:
            rec.experience_years = relativedelta(today, rec.license_date).years if rec.license_date else 0
